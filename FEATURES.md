...
- **Keep Languages Up to Date**: Refreshes the languages already installed when a new Sparkle release publishes newer packs, so the screens a release adds are translated instead of falling back to English. It checks at most once a day, in the background, and only ever downloads languages already installed; the row carries the date of the last successful check. A pack imported from a file is never replaced, and a failed check backs off for an hour rather than retrying on every launch.
